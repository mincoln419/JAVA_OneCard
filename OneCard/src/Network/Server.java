         ...
         try {
            while(din!=null){
               ...
               if(arr[0]=='1'){
                ...
                }else if(arr[0]=='^'){//한장 남은 사람이 원카드 버튼을 눌렀을 때                	
              	   msg=msg.substring(1, arr.length);            	   
              	   sendChance(msg,onecnt);
              	   onecnt++;
                }else if(arr[0]=='*'){//원카드를 저지할 때             	   
              	   msg=msg.substring(1, arr.length);            	   
              	   sendSad(msg,onecnt);   
             	   onecnt++;
...
////////////////////////////////////여기부터 매소드
		...
		private void sendOneBtn() {
			Iterator<String> it = clientsMap.keySet().iterator();
			
	   	      while(it.hasNext()){
	   	         try {
	   	            key=it.next();
	   	            clientsMap.get(key).writeUTF("m");
		...
		private void sendWinner(String msg) {
			Iterator<String> it = clientsMap.keySet().iterator();
			
   	      while(it.hasNext()){
   	         try {
   	            key=it.next();
   	            clientsMap.get(key).writeUTF("z"+msg);
   	         } catch (SocketException e){        	 
   	             System.out.println("서버에러");
   	         } catch (IOException e) {
   	            e.printStackTrace();
		...
		private void sendSad(String msg, int su) {/////////////방해공작으로 원카드 먼저 부르는데 실패
			Iterator<String> it = clientsMap.keySet().iterator();
			if(su==0){
   	      while(it.hasNext()){
   	         try {
		...
		private void sendChance(String msg, int su) {/////////////원카드 먼저 부르는데 성공
			Iterator<String> it = clientsMap.keySet().iterator();
			if(su==0){
   	      while(it.hasNext()){
   	         try {
	...
	private synchronized void sendCard(String cardNum) {
		 Iterator<String> it = clientsMap .keySet().iterator();
